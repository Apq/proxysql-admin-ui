...
- 修改前重新验证 Runtime 成员身份、节点身份、集群身份和旧权重。
- 修改后重新读取节点并验证新权重和集群 UUID。
...
对应代码属性为 `GaleraNodeWeightViewModel.IsHealthy`。

这些状态仅用于运维参考，不作为权重修改的前置条件或修改后成功条件。当前实现不会读取其他节点来确认整个集群是否健康，也不会根据集群规模评估权重修改后的仲裁风险。
...
6. 如果 `SHOW GRANTS` 明确识别到全局写权限，则显示已识别；无法识别时仍由实际写命令判断。
7. 拒绝与当前值相同的新权重。
8. 参数化执行单项 Provider Option 更新。
9. 重新读取目标节点。
10. 验证读取到的新权重等于请求值。
11. 验证集群 UUID 没有变化。
12. 记录成功日志并返回修改前后状态。
...
- `Primary / Synced / Ready / Connected` 和 WSREP 状态只展示，不阻止修改。
- 新权重只接受 `0..255`，且不能等于当前值。
- 修改命令只提交 `pc.weight=<value>`。
- 修改后必须重新读取并验证新权重和集群 UUID。
