...
ProxySQL Admin UI 是一个基于 Blazor Server 和 .NET 8 的 ProxySQL Web 管理界面。应用通过 ProxySQL Admin 接口读取配置和运行统计，不直接连接后端 MySQL 业务库。
...
> [!CAUTION]
> 本应用可以修改线上 ProxySQL 的路由和认证配置。生产环境请限制 Web 界面访问来源，使用专用 ProxySQL Admin 账号，并在执行写操作前确认回滚方案。
...
应用不会从环境变量读取固定的 Web 初始密码，也不支持 `PAI_DefaultUsers__0__Password`。已有身份数据库不会在容器重启或升级时重置管理员密码。
...
| 环境变量 | 必需 | 说明 |
| --- | --- | --- |
| `PAI_ConnectionStrings__ProxySqlContext` | 是 | ProxySQL Admin 连接字符串，通常使用 Admin 端口 `6032` |
| `PAI_DefaultUsers__0__Username` | 否 | 首次初始化的 Web 管理员用户名，默认 `admin` |
